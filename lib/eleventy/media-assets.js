...
const { deliveryAddressIndex, readMergedManifest, storedObjectKey } = require("../media-manifest");
...
function createMediaAssetHelpers(options = {}) {
  ...
  // The manifest key identifies the asset; the object key addresses its bytes, and since the
  // move to content-addressed uploads the two are no longer the same for anything written
  // recently. Building the URL from the entry rather than from the path is what makes a
  // replaced image resolve to its new object instead of to a cached copy of the old one.
  function toDeliveryUrl(publicPath = "") {
    for (const [assetPrefix] of deliveryPathPrefixes) {
      if (!publicPath.startsWith(assetPrefix)) continue;
      const manifestKey = objectKeyForPublicPath(publicPath);
      const entry = mediaManifest[manifestKey];
      if (!entry) return publicPath;
      return `https://${deliveryHost}/${storedObjectKey(entry, manifestKey)}`;
    }
    return publicPath;
  }

  // Was das Manifest über eine lokale Datei schon weiss.
  //
  // Der Bau las bisher für jedes Bild die echten Bytes — dreimal: für die Abmessungen, für den
  // Hash, aus dem die Dateinamen der responsiven Varianten entstehen, und für den unscharfen
  // Platzhalter. Abmessungen und Hash stehen aber längst im Manifest; nur der Platzhalter wurde
  // jedes Mal neu gerechnet.
  //
  // Weil die Bytes seit der Auslagerung nach R2 nicht mehr im Checkout liegen, hiess das: 294 MB
  // in 1158 Dateien vor jedem Bau herunterladen, um daraus 222 KB Platzhalter zu erzeugen und
  // zwei bekannte Werte nachzurechnen. Das war der grösste Posten im Bau (43 s von 52 s).
  //
  // Der Index steht hier einmal, nicht je Referenz: Das Manifest hat über 6.000 Einträge, und
  // diese Fragen werden für jedes Bild jeder Seite gestellt.
  let quellPfadIndex = null;
  function manifestEintragFuer(localPath) {
    if (!quellPfadIndex) {
      quellPfadIndex = new Map();
      for (const eintrag of Object.values(mediaManifest)) {
        if (eintrag?.sourcePath) quellPfadIndex.set(path.resolve(root, eintrag.sourcePath), eintrag);
      }
    }
    return quellPfadIndex.get(path.resolve(localPath)) || null;
  }
  ...
  // Der Rückweg: von einer Auslieferungs-URL auf die lokale Datei.
  //
  // Für die Pfadform genügt eine Prefix-Regel (/images/… → /assets/images/…). Für eine
  // Inhaltsadresse kann es die nicht geben — cas/<hash> sagt nichts über den Pfad, das steht
  // nur im Manifest. Ohne diesen Index bekäme ein Bild, das im Markdown unter seiner
  // Inhaltsadresse steht, weder Abmessungen noch responsive Varianten noch ein Platzhalterbild;
  // es bliebe ein nacktes img-Element mit einer fremden URL.
  //
  // Abgelöste Adressen zeigen mit: sie stehen weiterhin in veröffentlichten Beiträgen und
  // müssen dieselbe lokale Datei finden.
  //
  // Einmal gebaut, nicht pro Referenz: das Manifest hat über 6.000 Einträge, und diese Funktion
  // läuft für jedes Bild jeder Seite.
  // Einmal gebaut, nicht pro Referenz: das Manifest hat über 6.000 Einträge, und diese
  // Funktion läuft für jedes Bild jeder Seite.
  let deliveryKeyIndex = null;

  function manifestKeyForDeliveryPathname(pathname = "") {
    if (!deliveryKeyIndex) deliveryKeyIndex = deliveryAddressIndex(mediaManifest);
    const manifestKey = deliveryKeyIndex.get(pathname.replace(/^\//, ""));
    return manifestKey ? `/${manifestKey}` : pathname;
  }
  ...
  function getLocalAssetPublicPath(src = "", publicPrefix = "") {
    ...
    try {
      ...
      pathname =
        host === deliveryHost
          ? fromDeliveryPathname(manifestKeyForDeliveryPathname(url.pathname))
          : url.pathname;
    } catch {
  ...
  function getLocalAssetInfo(src = "", rootDirectory, publicPrefix = "", defaultDirectories = []) {
    ...
    if (pathname) {
      ...
      // Die Datei auf der Platte war bisher der Beleg dafür, dass es diese Mediendatei gibt.
      // Seit der Bau ein vollständig beschriebenes Bild nicht mehr herunterlädt, ist sie es
      // nicht mehr allein — ein Eintrag im Manifest belegt dasselbe. Ohne diese zweite Quelle
      // gälte ein nicht geladenes Bild als „kein lokales Medium", bekäme keine Auslieferungs-URL
      // und würde von mysite.example statt von media.mysite.example geholt. check-media-delivery.js hat
      // genau das gefangen.
      if (!fs.existsSync(localPath) && !manifestEintragFuer(localPath)) return null;
    ...
    for (const candidate of getRelativeAssetCandidates(src, defaultDirectories)) {
      ...
      if (!fs.existsSync(localPath) && !manifestEintragFuer(localPath)) continue;
  ...
  async function getLocalImageDimensions(src = "") {
    ...
    if (!imageDimensionsCache.has(imagePath)) {
      // Aus dem Manifest, wenn es sie kennt — einschliesslich `hasAlpha`. Das ist kein
      // Nebenwert: Bei einem durchsichtigen Bild bleibt der unscharfe Platzhalter absichtlich
      // weg, sonst schiene er durch. Ein hart gesetztes `false` gab 22 durchsichtigen Bildern
      // einen Platzhalter, den sie vorher zu Recht nicht hatten. Fehlt die Angabe im Eintrag,
      // wird nicht geraten, sondern wie bisher aus den Bytes gelesen.
      const bekannt = manifestEintragFuer(imagePath);
      if (bekannt?.width && bekannt?.height && typeof bekannt.hasAlpha === "boolean") {
        imageDimensionsCache.set(
          imagePath,
          Promise.resolve({ width: bekannt.width, height: bekannt.height, hasAlpha: bekannt.hasAlpha })
        );
  ...
  async function getImageLqip(asset) {
    if (!imageLqipCache.has(asset.localPath)) {
      // Der einzige der drei Werte, der bisher nirgends stand. Er wird beim Hochladen einmal
      // gerechnet und im Manifest abgelegt; hier wird er nur noch nachgeschlagen. Fehlt er —
      // ein frisch hochgeladenes Bild, dessen Eintrag noch keinen trägt —, wird er wie bisher
      // aus den Bytes gerechnet.
      const bekannt = manifestEintragFuer(asset.localPath);
      if (bekannt?.lqip) {
        imageLqipCache.set(asset.localPath, Promise.resolve(bekannt.lqip));
  ...
  async function getFileHash(filePath = "") {
    if (!fileHashCache.has(filePath)) {
      // Derselbe Wert, nur schon berechnet: Das Manifest führt den vollen sha256, und die
      // Variantennamen benutzen dessen erste zwölf Zeichen.
      const bekannt = manifestEintragFuer(filePath);
      if (bekannt?.sha256) {
        fileHashCache.set(filePath, Promise.resolve(bekannt.sha256.slice(0, 12)));
