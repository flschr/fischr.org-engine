const { addHtmlClassesToTag, stripHtml } = require("./html");
...
function normalizeAdmonitionHeadings(content = "") {
  return String(content).replace(/<blockquote\b([^>]*)>([\s\S]*?)<\/blockquote>/gi, (match, attrs, inner) => {
    const firstTitle = inner.match(/<h([4-6])\b[^>]*>[\s\S]*?<\/h\1>/i);
    if (!firstTitle) return match;
    ...
    const normalized = inner.replace(/<h([4-6])\b[^>]*>([\s\S]*?)<\/h\1>/gi, (heading, level, headingContent) => {
      return `<p class="admonition-title">${headingContent}</p>`;
    });
    const openTag = addHtmlClassesToTag(`<blockquote${attrs}>`, ["admonition", `admonition-${admonitionType}`]);

    return `${openTag}${normalized}</blockquote>`;
  });
}
