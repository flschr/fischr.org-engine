...
const { applyPathDelta, checkoutBranch, countChanges, diffPaths, git, gitQuiet, rev, run, writeOutput } = require("./lib/publish-git");
...
function assertVideoDerivativesPrepared() {
  run("node", ["scripts/generate-video-posters.js"], { stdio: "inherit" });
  if (!gitQuiet(["diff", "--quiet", "--", "blog/assets/images/video-posters", "blog/_data/videoMetadata.json"])) {
    throw new Error("Video posters and metadata must be generated on drafts before review. Re-open the admin and retry.");
  }
}
