...
test("unfinished media blocks publishing and destructive queue actions", () => {
  ...
  assert.match(source, /await waitForMediaCommits\(\);\s*if \(!guardMediaIdle\("Löschen"\)\) return/);
  ...
  assert.match(source, /const confirmedChanges = await loadFreshChanges\(\);\s*if \(!guardMediaIdle\("Verwerfen"\)\) return/);
