...
test("unfinished media blocks publishing and destructive queue actions", () => {
  ...
  assert.match(source, /await waitForMediaCommits\(\);\s*if \(!guardMediaIdle\(t\("action\.deleting"\)\)\) return/);
  ...
  assert.match(source, /if \(!guardMediaIdle\(t\("action\.discardAllChanges"\)\)\) return/);
  ...
  assert.match(source, /const confirmedChanges = await loadFreshChanges\(\);\s*if \(!guardMediaIdle\(t\("action\.discard"\)\)\) return/);
