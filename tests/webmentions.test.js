...
test("extracts outbound Webmention targets from post markdown", () => {
  const targets = extractTargetUrls(
    `
[Own link](https://example.com/eigener-artikel/)
...
test("normalizes target urls and skips unsupported or local links", () => {
  const excludeHosts = new Set(["example.com"]);
  ...
  assert.equal(normalizeTargetUrl("https://www.example.com/local", { excludeHosts }), "");
});
...
test("waits when the deployed source has no mf2 entry yet", async () => {
  const targets = await getPublishedMf2Targets("https://example.com/post/", {
    fetch: async (url) => ({
      ok: true,
      url,
      headers: headers({ "content-type": "text/html" }),
...
test("posts source and target to the Webmention endpoint", async () => {
  const result = await sendWebmention("https://example.com/webmention", "https://example.com/post/", "https://example.com/post", {
    fetch: async (url, options) => {
      assert.equal(url, "https://example.com/webmention");
      assert.equal(options.method, "POST");
      assert.equal(options.headers["Content-Type"], "application/x-www-form-urlencoded");
      assert.equal(options.body, "source=https%3A%2F%2Fexample.com%2Fpost%2F&target=https%3A%2F%2Fexample.com%2Fpost");
      return {
        ok: true,
        status: 202,
        text: async () => ""
...
test("exposes rejected Webmention responses as typed HTTP errors", async () => {
  await assert.rejects(
    sendWebmention("https://example.com/webmention", "https://example.com/post/", "https://example.com/post", {
      fetch: async () => ({
        ok: false,
        status: 400,
